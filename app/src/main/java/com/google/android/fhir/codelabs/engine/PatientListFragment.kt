...
class PatientListFragment : Fragment() {
  ...
  private fun handleSyncJobStatus(syncJobStatus: SyncJobStatus) {when (syncJobStatus) {
    is SyncJobStatus.Finished -> {
      Toast.makeText(requireContext(), "Sync Finished(from handlesyncjob)", Toast.LENGTH_SHORT).show()
      ...
      object : MenuProvider {
        ...
        override fun onMenuItemSelected(menuItem: MenuItem): Boolean {
          return when (menuItem.itemId) {
            R.id.sync -> {
              Toast.makeText(requireContext(), "Sync clicked", Toast.LENGTH_LONG).show()
